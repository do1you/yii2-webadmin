/**
 ...
 @Date：2015-1-12 
 */
define('pui/rating',['jquery', 'pui/rating.css'], function(require, exports, moudles) {
	moudles.exports = (function($) {
		...
		$.fn.rating = function(options) {
			...
			if (!this.is('input') && this.find('input').length) {
				return $.fn.rating.apply(this.find('>input'),arguments);
			}
			...
			var options = $.extend({} /* new object */ , $.fn.rating.options /* default options */ , options || {} /* just-in-time options */ );
			...
			this.not('.star-rating-applied').addClass('star-rating-applied').each(function() {
				...
				//}// save a byte!
					...
					control = $.extend({} /* new object */ , options || {} /* current call options */ , ($.metadata ? input.metadata() : ($.meta ? input.data() : null)) || {}, /* metadata options */ {
						count: 0,
						stars: [],
						inputs: []
					});
					...
					rater.append(
					control.cancel = $('<div class="rating-cancel"><a title="' + control.cancel + '">' + control.cancelValue + '</a></div>').on('mouseover', function() {
						$(this).rating('drain');
		...
		*/
		$.extend($.fn.rating, {
			...
			draw: function() {
				...
				//? control.current.data('rating.input') : null );
