! function (n) {
    var t = function (t, i) {
        this.$element = n(t);
        this.options = n.extend({}, n.fn.tree.defaults, i);
        this.$element.on("click", ".tree-item", n.proxy(function (u) {
            this.selectItem(u.currentTarget);
        }, this));
        this.$element.on("click", ".tree-folder-header", n.proxy(function (u) {
			if(n(u.target).is('i')){
				this.toggleFolder(u.currentTarget);
			}else{
				n(u.currentTarget).find('.fa.fa-folder').length ? this.toggleFolder(u.currentTarget) : this.selectFolder(u.currentTarget);
			}
        }, this));
        this.render();
    };
    t.prototype = {
        constructor: t,
        render: function () {
            this.populate(this.$element)
        }, populate: function (t) {
            var i = this,
                r = t.parent(),
                u = r.find(".tree-loader:eq(0)");
            u.show();
            this.options.dataSource.data(t.data(), function (f) {
                u.hide();
                n.each(f.data, function (u, f) {
                    var e, o;
                    "folder" === f.type ? (e = i.$element.find(".tree-folder:eq(0)").clone().show(), e.find(".tree-folder-name").html(f.name), e.find(".tree-loader").html(i.options.loadingHTML), e.find(".tree-folder-header").data(f), e.find('[class*="fa-"]').addClass(f["icon-class"]), e.data(f)) : "item" === f.type && (e = i.$element.find(".tree-item:eq(0)").clone().show(), e.find(".tree-item-name").html(f.name), e.data(f));
                    o = f.dataAttributes || [];
                    n.each(o, function (n, t) {
                        switch (n) {
                        case "class":
                        case "classes":
                        case "className":
                            e.addClass(t);
                            break;
                        default:
                            e.attr(n, t)
                        }
                    });
					t.hasClass("tree-folder-header") ? r.find(".tree-folder-content:eq(0)").append(e) : t.append(e);
					f.selected ? ("folder" === f.type ? i.selectFolder(e.find(">.tree-folder-header")) : i.selectItem(e)) : e.parents('.tree-folder').each(function(){
						var p = n(this);
						p.find('.tree-folder,.tree-item').not('.tree-selected').length ? p.addClass('tree-part') : p.removeClass('tree-part');
					});
                });
                i.$element.trigger("loaded", r)
            })
        }, selectItem: function (t) {
            var i = n(t),
                u = this.$element.find(".tree-selected"),
                r = [],
				p = i.parents('.tree-folder'),
                f;
            (this.options.multiSelect ? n.each(u, function (t, u) {
                var f = n(u);
                f[0] !== i[0] && r.push(n(u).data())
            }) : u[0] !== i[0] && (u.removeClass("tree-selected").find("i").removeClass("fa fa-check-square-o").addClass("tree-dot"), r.push(i.data())), this.options.selectable) && (f = "selected", i.hasClass("tree-selected") ? (f = "unselected", i.removeClass("tree-selected"), i.find("i").removeClass("fa fa-check-square-o").addClass("tree-dot")) : (i.addClass("tree-selected"), i.find("i").removeClass("tree-dot").addClass("fa fa-check-square-o"), this.options.multiSelect && r.push(i.data())));
            r.length && this.$element.trigger("selected", {
                info: r
            });
			this.options.multiSelect && p.each(function(){
				var p = n(this);
				p.find('>.tree-folder-content>.tree-selected').length ? p.addClass('tree-selected') : p.removeClass('tree-selected');
				p.find('.tree-folder,.tree-item').not('.tree-selected').length ? p.addClass('tree-part') : p.removeClass('tree-part');
			}); // 子项选中父项同时选中
            i.trigger("updated", {
                info: r,
                item: i,
                eventType: f
            });
        }, selectFolder: function (t) {
            var i = n(t),
				u = this.$element.find(".tree-selected"),
                o = i.parent(),
				p = o.parents('.tree-folder').add(o.find('.tree-folder')),
				r = [],
                s = o.find(".tree-folder-content"),
                e = s.find('.tree-folder,.tree-item');
           r.push(i.data());
		   if(this.options.multiSelect){
			    o.removeClass('tree-part').toggleClass('tree-selected');
			    e.each(function(t,u){
					r.push(n(u).data());
					o.is('.tree-selected') ? n(u).addClass('tree-selected').find("i").removeClass("tree-dot").addClass("fa fa-check-square-o") : n(u).removeClass('tree-selected').find("i").removeClass("fa fa-check-square-o").addClass("tree-dot");
			    });
				p.each(function(){
					var p = n(this);
					p.find('>.tree-folder-content>.tree-selected').length ? p.addClass('tree-selected') : p.removeClass('tree-selected');
					p.find('.tree-folder,.tree-item').not('.tree-selected').length ? p.addClass('tree-part') : p.removeClass('tree-part');
				});
		   }else{
				u.not(o).removeClass("tree-selected tree-part").find("i").removeClass("fa fa-check-square-o").addClass("tree-dot");
				o.toggleClass('tree-selected');
		   }
		   i.trigger("updated", {
                info: r,
                item: i,
                eventType: (o.hasClass('tree-selected') ? 'selected' : 'unselected')
            });
        }, toggleFolder: function (t) {
            var r, u, f, i = n(t),
                o = i.parent(),
                s = o.find(".tree-folder-content"),
                e = s.eq(0);
            i.find(".fa.fa-folder").length ? (r = "opened", u = ".fa.fa-folder", f = "fa fa-folder-open", e.show(), s.children().length || this.populate(i)) : (r = "closed", u = ".fa.fa-folder-open", f = "fa fa-folder", e.hide(), this.options.cacheItems || e.empty());
            o.find(u).eq(0).removeClass("fa fa-folder fa-folder-open").addClass(f);
            this.$element.trigger(r, i.data());
        }, selectedItems: function () {
            var i = this.$element.find(".tree-selected"),
                t = [];
            return n.each(i, function (i, r) {
                t.push(n(r).data());
            }), t
        }, collapse: function () {
            var t = this.options.cacheItems;
            this.$element.find(".fa.fa-folder-open").each(function () {
                var r = n(this).removeClass("fa fa-folder fa-folder-open").addClass("fa fa-folder"),
                    u = r.parent().parent(),
                    i = u.children(".tree-folder-content");
                i.hide();
                t || i.empty();
            });
        }, expand: function () {
			var t = this;
            t.$element.find(".fa.fa-folder:gt(0)").each(function () {
                var i = n(this).parent().parent().children(".tree-folder-header");
                t.toggleFolder(i);
            });
			t.$element.find(".fa.fa-folder:gt(0)").length && t.expand();
        }
    };
    n.fn.tree = function (i, r) {
        var u, f = this.each(function () {
            var e = n(this),
                f = e.data("tree"),
                o = "object" == typeof i && i;
            f || e.data("tree", f = new t(this, o));
            "string" == typeof i && (u = f[i](r))
        });
        return void 0 === u ? f : u
    };
    n.fn.tree.defaults = {
        selectable: !0,
        multiSelect: !1,
        loadingHTML: "<div>Loading...<\/div>",
        cacheItems: !0
    };
    n.fn.tree.Constructor = t
}(window.jQuery);