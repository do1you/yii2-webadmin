...
;(function ($, window, document, undefined) {
  ...
  var pluginName = 'bootstrapDualListbox',
    defaults = {
      ...
      infoText: '可选项 ({0})',                                       // text when all options are visible / false for no info text
      infoTextFiltered: '<span class="label label-warning">查询结果</span> {0}/{1}', // when not all of the options are visible due to the filter
      infoTextEmpty: '暂无可选项',                                                        // when there are no options present in the list
	  infoText2: '已选项 ({0})',
  ...
  function refreshInfo(dualListbox) {
    ...
    if (all2 === 0) {
      ...
      content = formatString(dualListbox.settings.infoText2, [visible2, all2]);
    } else {
  ...
  BootstrapDualListbox.prototype = {
    init: function () {
	  ...
	  if(this.element.attr('readonly') || this.element.attr('disabled')){
		  this.elements.select1.off('change').attr('disabled',true);
		  this.elements.select2.off('change').attr('disabled',true);
		  this.setShowFilterInputs(false);
		  this.elements.box1.find('.buttons').hide();
		  this.elements.box2.find('.buttons').hide();
	  }else{
		  bindEvents(this);
	  }
      refreshSelects(this);
