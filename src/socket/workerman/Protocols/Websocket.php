...
class Websocket implements \Workerman\Protocols\ProtocolInterface
{
        ...
        if ($connection->context->websocketCurrentFrameLength) {
            ...
            switch ($opcode) {
                ...
                case 0x8:
                    ...
                    $close_cb = $connection->onWebSocketClose ? $connection->onWebSocketClose : ($connection->worker->onWebSocketClose ? $connection->worker->onWebSocketClose : false);
                    if ($close_cb) {
                        try {
                            $close_cb($connection);
                        } catch (\Throwable $e) {
            ...
            if ($is_fin_frame) {
                if ($opcode === 0x9) {
                    if ($recv_len >= $current_frame_length) {
                        ...
                        $ping_cb = $connection->onWebSocketPing ? $connection->onWebSocketPing : ($connection->worker->onWebSocketPing ? $connection->worker->onWebSocketPing : false);
                        if ($ping_cb) {
                            try {
                    ...
                    if ($recv_len >= $current_frame_length) {
                        ...
                        $pong_cb = $connection->onWebSocketPong ? $connection->onWebSocketPong : ($connection->worker->onWebSocketPong ? $connection->worker->onWebSocketPong : false);
                        if ($pong_cb) {
                            try {
        ...
        if (empty($connection->context->websocketHandshake)) {
            ...
            if (\strlen($connection->context->tmpWebsocketData) > $connection->maxSendBufferSize) {
                if ($connection->onError) {
                    try {
                        \call_user_func($connection->onError, ConnectionInterface::SEND_FAIL, 'send buffer full and drop package');
    ...
    public static function dealHandshake($buffer, $connection)
    {
        ...
        if (0 === \strpos($buffer, 'GET')) {
            ...
            $on_websocket_connect = $connection->onWebSocketConnect ? $connection->onWebSocketConnect : ($connection->worker->onWebSocketConnect ? $connection->worker->onWebSocketConnect : false);
