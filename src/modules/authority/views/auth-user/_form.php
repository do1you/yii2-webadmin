...
<div class="row auth-user-form">
    <div class="col-lg-offset-3 col-sm-offset-1 col-lg-6 col-sm-10 col-xs-12">
        ...
        <?= $form->field($model, 'fail_num')->textInput(['maxlength' => true]) ?>

        <?= $form->field($model, 'unlock_time')->textInput(['maxlength' => true])->hint('值为UNIX时间戳，0：未锁定，-1：永久锁定。当前状态：' . ($model->unlock_time == 0 ? '未锁定' : ($model->unlock_time == -1 ? '永久锁定' : '于' . date('Y-m-d H:i:s', $model->unlock_time) . '解锁'))) ?>
