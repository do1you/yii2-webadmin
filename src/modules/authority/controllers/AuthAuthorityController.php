<?php
/**
 * 模型对象AuthAuthority的增删改查控制器方法.
 */ 
namespace webadmin\modules\authority\controllers;

use Yii;
use webadmin\modules\authority\models\AuthAuthority;
use yii\data\ActiveDataProvider;

class AuthAuthorityController extends \webadmin\BController
{
	// 执行前
    public function beforeAction($action){
        Yii::$app->controller->pageTitle = Yii::t('authority', '权限定义管理');
    ...
    public function actionIndex()
    {
		...
		$model = new AuthAuthority();
        $dataProvider = $model->search(Yii::$app->request->queryParams,[],'parent');
    ...
    public function actionTree()
    {
        ...
        return $this->render('tree', [
            'treeData' => AuthAuthority::treeData(),
        ]);
    }
    ...
    public function actionCreate()
    {
        ...
        $model->parent_id = Yii::$app->request->get('id','0');
    ...
    public function actionDelete()
    {
        ...
        if($id && ($models = AuthAuthority::findAll($id))){
            $transaction = AuthAuthority::getDb()->beginTransaction(); // 使用事务关联
    ...
    {
        if (($model = AuthAuthority::findOne($id)) !== null) {
            return $model;
        }
