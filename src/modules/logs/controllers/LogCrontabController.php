<?php
/**
 * 模型对象LogCrontab的增删改查控制器方法.
 */ 
namespace webadmin\modules\logs\controllers;

use Yii;
use webadmin\modules\logs\models\LogCrontab;
use yii\data\ActiveDataProvider;

class LogCrontabController extends \webadmin\BController
{
	// 执行前
    public function beforeAction($action){
        Yii::$app->controller->pageTitle = Yii::t('logs', '计划任务日志');
    ...
    */
    ...
    /*public function actionDelete()
    {
        ...
        if($id && ($models = LogCrontab::findAll($id))){
        	$transaction = LogCrontab::getDb()->beginTransaction(); // 使用事务关联
    ...
    {
        if (($model = LogCrontab::findOne($id)) !== null) {
            return $model;
        }
