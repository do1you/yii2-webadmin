<?php
/**
 * 模型对象LogUserAction的增删改查控制器方法.
 */ 
namespace webadmin\modules\logs\controllers;

use Yii;
use webadmin\modules\logs\models\LogUserAction;
use yii\data\ActiveDataProvider;

class LogUserActionController extends \webadmin\BController
{
	// 执行前
    public function beforeAction($action){
        Yii::$app->controller->pageTitle = Yii::t('logs', '操作日志');
    ...
    public function actionTree()
    {
        ...
        return $this->render('tree', [
            'treeData' => LogUserAction::treeData(),
        ]);
    }
    */
    ...
    /*public function actionDelete()
    {
        ...
        if($id && ($models = LogUserAction::findAll($id))){
        	$transaction = LogUserAction::getDb()->beginTransaction(); // 使用事务关联
    ...
    {
        if (($model = LogUserAction::findOne($id)) !== null) {
            return $model;
        }
