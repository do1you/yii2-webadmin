<?php
/**
 * 模型对象LogImei的增删改查控制器方法.
 */ 
namespace webadmin\modules\logs\controllers;

use Yii;
use webadmin\modules\logs\models\LogImei;
use yii\data\ActiveDataProvider;

class LogImeiController extends \webadmin\BController
{
	// 执行前
    public function beforeAction($action){
        Yii::$app->controller->pageTitle = Yii::t('logs', '用户终端');
    ...
    */
    ...
    public function actionDelete()
    {
        ...
        if($id && ($models = LogImei::findAll($id))){
            $transaction = LogImei::getDb()->beginTransaction(); // 使用事务关联
    ...
    {
        if (($model = LogImei::findOne($id)) !== null) {
            return $model;
        }
