<?php
/**
 * 模型对象LogSms的增删改查控制器方法.
 */ 
namespace webadmin\modules\logs\controllers;

use Yii;
use webadmin\modules\logs\models\LogSms;
use yii\data\ActiveDataProvider;

class LogSmsController extends \webadmin\BController
{
	// 执行前
    public function beforeAction($action){
        Yii::$app->controller->pageTitle = Yii::t('logs', '短信日志');
    ...
    */
    ...
    /*public function actionDelete()
    {
        ...
        if($id && ($models = LogSms::findAll($id))){
        	$transaction = LogSms::getDb()->beginTransaction(); // 使用事务关联
    ...
    {
        if (($model = LogSms::findOne($id)) !== null) {
            return $model;
        }
