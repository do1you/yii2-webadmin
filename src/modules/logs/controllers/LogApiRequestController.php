<?php
/**
 * 模型对象LogApiRequest的增删改查控制器方法.
 */ 
namespace webadmin\modules\logs\controllers;

use Yii;
use webadmin\modules\logs\models\LogApiRequest;
use yii\data\ActiveDataProvider;

class LogApiRequestController extends \webadmin\BController
{
	// 执行前
    public function beforeAction($action){
        Yii::$app->controller->pageTitle = Yii::t('logs', '接口请求日志');
    ...
    public function actionTree()
    {
        ...
        return $this->render('tree', [
            'treeData' => LogApiRequest::treeData(),
        ]);
    }
    */
    ...
    /*public function actionDelete()
    {
        ...
        if($id && ($models = LogApiRequest::findAll($id))){
        	$transaction = LogApiRequest::getDb()->beginTransaction(); // 使用事务关联
    ...
    {
        if (($model = LogApiRequest::findOne($id)) !== null) {
            return $model;
        }
