...
class Image_GD_Driver extends Image_Driver {
	...
	public function resize($properties)
	{
		...
		{
			$properties['width'] = min($properties['width'],$width);
			$properties['height'] = min($properties['height'],$height);
			$w = ($width / $properties['width']);
			$h = ($height / $properties['height']);
			$isCrop = max($w,$h)/min($w,$h)>=3;
			$cropProperties = array(
			...
			if($isCrop){
				$properties['master'] = ($w > $h) ? Image::HEIGHT : Image::WIDTH;
			}else{
				$properties['master'] = ($w > $h) ? Image::WIDTH : Image::HEIGHT;
			}
		}
		...
		// crop
		if(!empty($isCrop) && !empty($cropProperties)){
			$this->crop($cropProperties);
		}
  ...
  public function watermark() {
    $watermark = imagecreatefrompng(WEBSITE_DIR.WATERMARK);
    $watermark_width = imagesx($watermark);
    $watermark_height = imagesy($watermark);
    $dest_x = imagesx($this->tmp_image)   - $watermark_width - 10;
    $dest_y = imagesy($this->tmp_image)   - $watermark_height - 10;
    imagealphablending($this->tmp_image, true);
    imagealphablending($watermark, true);
    return imagecopy($this->tmp_image, $watermark, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height);
  }
